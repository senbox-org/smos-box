...
import ucar.nc2.iosp.smos.bufr.BufrDataSection;
...
public class BufrSplitter {
    ...
    Options options;
    File dirOut;
    ucar.nc2.iosp.smos.bufr.writer.MessageDispatchDDS dispatcher;
    ...
    void processMessageTask(MessageTask mtask) {
        ...
        try {
            ucar.nc2.iosp.smos.bufr.Message m = getMessage(new InMemoryRandomAccessFile("BUFR", mtask.mess));
            if (null == m) return;
    ...
    public ucar.nc2.iosp.smos.bufr.Message getMessage(RandomAccessFile raf) throws IOException {
        raf.seek(4);
        ucar.nc2.iosp.smos.bufr.BufrIndicatorSection is = new ucar.nc2.iosp.smos.bufr.BufrIndicatorSection(raf);
        ucar.nc2.iosp.smos.bufr.BufrIdentificationSection ids = new ucar.nc2.iosp.smos.bufr.BufrIdentificationSection(raf, is);
        ucar.nc2.iosp.smos.bufr.BufrDataDescriptionSection dds = new ucar.nc2.iosp.smos.bufr.BufrDataDescriptionSection(raf);

        long dataPos = raf.getFilePointer();
        int dataLength = ucar.nc2.iosp.smos.bufr.BufrNumbers.uint3(raf);
        ucar.nc2.iosp.smos.bufr.BufrDataSection dataSection = new BufrDataSection(dataPos, dataLength);
        ...
        return new ucar.nc2.iosp.smos.bufr.Message(raf, is, ids, dds, dataSection);
    }
